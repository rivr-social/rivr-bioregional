# rivr-region — pm-core module

RIVR Bioregional Commons App as a first-class **pm-core module**. Runs on the pm-core foundation
...
nothing instance-specific. The same module directory runs any `region` node:
...
1. **Drop in.** Copy this directory into your pm-core checkout at
   `<pm-core>/modules/rivr-region/` (it `extends:` `../../foundation/docker-compose.base.yml`,
   so it must sit two levels under the foundation root).
2. **Source.** Provide the app image one of two ways:
   - **Pinned image (recommended):** set `RIVR_IMAGE=rivr-region@sha256:<digest>` in
     `.env`. No build, no drift.
   - **Build from source:** clone/checkout https://github.com/rivr-social/rivr-bioregional into `./src` (so
...
4. **Secrets.** Create the file-based secrets listed in `secrets-required.txt`
   ...
   ```sh
   mkdir -p /opt/pm-core/secrets/region
   printf 'postgresql://rivr:PW@postgres:5432/rivr_region' > /opt/pm-core/secrets/region/db_url
   printf 'redis://:PW@redis:6379'                        > /opt/pm-core/secrets/region/redis_url
   ...
   chmod 644 /opt/pm-core/secrets/region/*
   ```
