...
/**
 * Reusable entity card used by the map card-rail and by graph node selection
 * overlays.
 *
 ...
 * navigates. `item.url` may be a canonical sovereign-home URL (a federated graph
 * node stamped with `homeHref`), so `CanonicalLink` renders a cross-origin target
 * as a plain `<a>` (no Next cross-origin RSC prefetch) and a local path as a Next
 * `<Link>`. It normalizes a heterogeneous entity
 * (`event`/`group`/`post`/`offering`/`person`) into one display shape and mirrors
 * the markup that previously lived inline in `app/(main)/map/page.tsx`, so map
