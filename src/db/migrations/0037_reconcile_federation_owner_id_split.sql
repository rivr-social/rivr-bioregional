-- 0037_reconcile_federation_owner_id_split.sql
...
-- minted a RANDOM UUID for a remote owner agent, while the manifest-reference
-- projection (manifest-references.ts) defaults `local_entity_id` to the origin's
-- EXTERNAL id. The two paths therefore disagreed on the local id for one remote
-- actor — a divergence that split ownership/attribution and broke "my content" /
-- delete gates. resolveLocalEntityId now keys NEW mappings by the external id so
-- the paths converge going forward; this script reconciles legacy rows already
-- on disk.
--
-- Strategy (safe + review-before-destroy):
--   * Only touches federated STUB agents (metadata federatedPlaceholder /
--     isProjection). Locally-owned / sovereign accounts are never moved.
--   * Repoints owned content + the entity-map pointer from the random-id stub
--     onto the external-id agent the projection canonically keys.
...
-- (this ecosystem applies migrations manually; chain is idempotent). This file
-- is intentionally NOT registered in meta/_journal.json.

-- 1) Repoint resources owned by the diverged random-id stub onto the external-id
--    canonical agent that the projection canonically keys.
WITH split AS (
  ...
  WHERE m.entity_type = 'agent'
    ...
    AND (
      stub.metadata->>'federatedPlaceholder' = 'true'
