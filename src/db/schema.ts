...
export const agents = pgTable(
  'agents',
  {
    ...
    // the next provisioning write. Never project this column into list/feed reads.
...
// ---------------------------------------------------------------------------
// Federated visitor access (browsing-SSO parity, migration 0036)
// ---------------------------------------------------------------------------

/**
 * Owner-configurable policy for cross-instance SSO visitors.
 *
 * Single-row-per-instance (mirrors peer_smtp_config). Controls whether
 * non-owner visitors landing via /api/federation/sso/land are auto-signed-in,
 * which capabilities beyond the baseline `read` they hold, their session TTL,
 * and whether landings are logged.
 */
 ...
 *
 * Mirrors mcp_provenance_log in append-only semantics. Each row records one
 * visitor landing: who came, from where, where they landed, and what scope
 * they were granted. Recording is gated by the owner's `recordVisits` setting.
 */
