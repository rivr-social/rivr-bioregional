 ...
 *             posting AS a group on another instance). The receiver accepts
 *             this and normalizes to its own local id.
 ...
 *      still rejected; identity is never invented. This REPLACES the prior
 *      `peerTrusted` shortcut that accepted the body-supplied `actorId` on the
 *      shared secret alone (F1 — sovereign mutation auth impersonation).
 *
...
async function authorizePeerSecret(
  ...
  // Peer-secret auth is server-to-server. Return only peerNodeId; the
  // mutations endpoint must STRICTLY bind the body-supplied actorId to a
  // federation_entity_map row for this peer (see bindAuthorizedFederationActor)
  // rather than trusting the claim on the shared secret alone.
