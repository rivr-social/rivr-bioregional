...
export async function ensureLocalActorAgent(actorId: string): Promise<void> {
  ...
  await db
    ...
    .values({
      ...
      // viewer-projection time (we only hold the signed session). The home peer's
      // next agent upsert upgrades this placeholder in place, so an org never
      // stays a person once its real type federates over.
      type: "person",
      visibility: "private",
      metadata: {
        ...
        // projection routes "My Profile" back to its sovereign home even when
        // no node-id (`sourceNodeId`) signal is available downstream.
