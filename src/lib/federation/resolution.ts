...
async function resolveHomeInstanceWithDepth(
  ...
  // Parent chain / metadata: check this agent's own home signal, then its parent.
  ...
  if (agent.length > 0) {
    ...
    // stamp (no locally-registered primary node) — e.g. a federated-viewer
    // projection (actor-projection). Consult it so the server-side home guard
    // agrees with the registry API and the client redirect (H1: profile buttons
    // not routing to the sovereign home). Loop guard inside: never resolve to
    // ourselves.
 ...
 *
 * Used when the only home signal we have is a `homeBaseUrl`/`canonicalUrl`
 * stamp (the remote-viewer cookie and federated stamps carry the URL, not the
 * node id). Returns null when no non-archived node is registered for that URL —
 * common on a sovereign, where a visitor's home is reached through the global
 * hub rather than registered locally. Shared with `ensureLocalActorAgent` so
 ...
 *
 * A federated-viewer projection may carry the home as a URL rather than a node
 * id. Prefers the explicit `homeBaseUrl`; otherwise falls back to the origin of
 * `canonicalUrl`. Returns null when no URL stamp is present, when it points back
 * at this instance (loop guard), or when no non-archived node is registered for
 * it.
 */
