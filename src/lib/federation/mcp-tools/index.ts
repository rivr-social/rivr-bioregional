...
export const MCP_TOOL_DEFINITIONS: McpToolDefinition[] = [
  {
    ...
    description: "Return the local Rivr bioregional instance identity and the authenticated actor context.",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description: "List knowledge graph documents for a scope. Defaults to region scope.",
    inputSchema: {
      type: "object",
      additionalProperties: false,
      properties: {
        scope_type: { type: "string", description: "Scope type (region, locale, group, person, persona, event, project). Default: region" },
    ...
    handler: async (args, context) => {
      const config = getInstanceConfig();
      const scopeType = getString(args.scope_type) ?? "region";
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        scope_type: { type: "string", description: "Scope type. Default: region" },
        scope_id: { type: "string", description: "Scope ID. Default: instance primary agent ID or current actor ID" },
    ...
    handler: async (args, context) => {
      ...
      const doc = await kg.createDoc({
        ...
        scope_id: scopeId,
        source_uri: `rivr://bioregional/resources/${resource.id}`,
      });
  ...
  {
    name: "rivr.kg.chat",
    description: "Chat with knowledge graph context. Fetches relevant KG facts for the bioregional scope and uses them to inform the response.",
    inputSchema: {
      type: "object",
      additionalProperties: false,
      required: ["message"],
