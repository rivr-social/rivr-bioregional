 ...
 *     class).
 *
 * Unlike the single-person app, this bioregional instance DOES host local
 * routes for every entity class it links to (`/profile/[username]`,
 * `/groups/[id]`, `/rings/[id]`, `/families/[id]`, `/projects/[id]`). So the
 * common case here is `globalFallback: false` — an entity with no federated
 * home stamp is assumed locally-homed and gets its bare local path, while a
 * federated projection (carrying a home stamp) routes to its true sovereign
 * home. `globalFallback: true` remains available for any class this instance
 * cannot render locally, routing an unstamped entity to the global aggregator
 * (`app.rivr.social`).
 *
 * The class of bug this module fixes: a federated projection (e.g. a Spirit
 * membership whose only home stamp is `federatedHomeBaseUrl`) was being linked
 * to a bare local path that renders the wrong thing or 404s, and cross-origin
 * links rendered as Next `<Link>` caused RSC-prefetch CSP flashes.
 *
...
export interface ResolveEntityHrefOptions {
   ...
   * verbatim. Pass `true` only for entity classes this instance cannot render
   * as a local page; leave `false` (the default here) for locally-renderable
   * classes — this bioregional app has local routes for every entity class it
   * links to, so a federated projection still routes to its true home while an
   * unstamped entity keeps its local path.
   */
