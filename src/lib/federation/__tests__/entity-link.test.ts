...
const SELF = "https://cascadia.rivr.social";
...
describe("resolveEntityHref", () => {
  ...
  it("local + globalFallback:false → verbatim local path, isRemote false", () => {
    const result = resolveEntityHref({}, "/rings/xyz", { globalFallback: false });
    expect(result).toEqual({ href: "/rings/xyz", isRemote: false });
  });
  ...
  it("defaults (no options) treat an unstamped entity as a verbatim local path", () => {
    expect(resolveEntityHref({}, "/rings/xyz")).toEqual({
      href: "/rings/xyz",
      isRemote: false,
    });
  });
