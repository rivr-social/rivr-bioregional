 ...
 * session) OR a federated viewer's raw home actorId (the `rivr_remote_viewer`
 * cookie carries the canonical home id verbatim — see
 * `src/lib/auth/get-session.ts`). A federated prime-agent (e.g. an admin homed
 * on a sovereign instance viewing their group on global) must still be
 * recognized as that group's admin. We never mint a separate local account;
 * instead we normalize the remote actorId to its canonical LOCAL agent id via
 * `federation_entity_map` and authorize the normalized identity.
 */
