 ...
 * matching `check()`'s locale semantics. Located viewers route locale resources
 * through the full permission check so resources owned in a non-shared locale
 * are not disclosed (GRP-SEC-003). Only non-public/non-locale items otherwise go
 * through the full permission check.
 */
