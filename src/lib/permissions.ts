 ...
 * an actor cannot set on themselves via profile edit.
 *
 * Two invariants depend on this set:
 * - `buildActorAttributes` never lets a `metadata` key shadow one of these
 *   authority columns, so self-asserted metadata cannot impersonate `id`/`type`
 *   or fabricate locale membership (`pathIds`).
 * - `evaluateAbacPolicies` only trusts these keys when an ALLOW policy would
 *   confer an elevated action (see `ELEVATED_ABAC_ACTIONS`).
 */
const SYSTEM_CONTROLLED_ATTRIBUTE_KEYS: ReadonlySet<string> = new Set([
  "id",
  "type",
  "pathIds",
]);
 ...
 * grants any of these is a privilege-escalation primitive if it can be
 * satisfied by a self-asserted (self-writable `agents.metadata`) attribute. For
 * such policies `evaluateAbacPolicies` evaluates conditions against ONLY the
 * authority-provenanced attributes (`SYSTEM_CONTROLLED_ATTRIBUTE_KEYS`), so a
 * user cannot grant themselves manage/grant/delete/etc. by editing their own
 * metadata (GRP-SEC-002 / DBR-SEC-001).
 *
