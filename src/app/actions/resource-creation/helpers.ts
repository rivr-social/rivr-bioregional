...
export async function createResourceWithLedger(input: CreateResourceInput): Promise<ActionResult> {
  ...
  try {
    ...
    // shape as the postedAsGroup/postedByAgentId provenance carried elsewhere.
    // This is what lets the controller see, moderate, and delete autobot-created
    // content (ownership is the delete/moderation gate; see canModifyResource).
    // PERSONAS keep distinct authorship (their own agent owns) and HUMANS are
    // unaffected. Applies only when the autobot would otherwise own the content
    // itself — never when an explicit group `input.ownerId` is in play (that
    // stays group-owned via the authorization gate below) — and falls back to
    // current behavior when no controllerId is available so we never null-own.
