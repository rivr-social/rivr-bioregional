 ...
 * (MCP tools, system prompts) where `auth()` does not resolve must pass
 ...
 * anonymous viewer (`viewerId` null) sees only publicly-crawlable (public/locale)
 * posts. Filtering happens BEFORE slicing so the limit is applied to the
 * viewer-visible set and private posts never consume an anonymous page budget.
 *
