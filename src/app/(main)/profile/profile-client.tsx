...
function getActivityObjectHref(
  ...
  // projection routes to its sovereign HOME. This bioregional app has a local
  // route for every entity class it links to, so `globalFallback` stays false
  // throughout: an unstamped entity keeps its local path while a home-stamped
  // projection routes to its true home. `selfBaseUrl` is omitted for
  // deterministic SSR/client output (no hydration mismatch) — a self-host stamp
  // simply resolves to the local path anyway.
  const canonical = (localPath: string): string =>
    resolveEntityHref(object.metadata, localPath, { globalFallback: false }).href;

  if (object.kind === "agent") {
    if (object.type === "organization") return canonical(`/groups/${object.id}`);
    if (object.type === "ring") return canonical(`/rings/${object.id}`);
    if (object.type === "family") return canonical(`/families/${object.id}`);
