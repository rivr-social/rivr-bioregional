...
/**
 * Explore hub page with two views: an Instagram-style image grid and a D3 force-directed graph.
 *
 * Route: `/explore` with optional query params:
 * - `q`: initial search term used to prefill the local search input.
 * - `tab`: initial tab selection ("gallery" or "graph").
 *
 * Data requirements:
 * - `useHomeFeed()` for groups and events graph data.
 * - `usePosts()` for active post feed data.
 * - App scope from `useAppContext()` (`selectedChapter`) for chapter-aware filtering.
 *
 * Rendering model:
 * - Client Component (`"use client"`) because it depends on client hooks, local state,
 *   browser APIs (`window`, `navigator`), and client navigation.
 */
...
import type { Post, Event, Group } from "@/lib/types"
...
/** 3D graph component loaded client-only (WebGL cannot run server-side). */
const ExploreGraph3D = dynamic(
  () => import("@/components/explore-graph-3d").then((mod) => mod.ExploreGraph3D),
  { ssr: false, loading: () => <div className="flex items-center justify-center py-20 text-muted-foreground">Loading 3D graph...</div> }
)
...
/**
 * Renders the explore experience with tabbed views: image gallery and force-directed graph.
 */
export default function ExplorePage() {
  ...
  const gridItems = useMemo<ExploreGridItem[]>(() => {
    ...
    // Collect groups with images (exclude placeholder images)
          ...
          {gridItems.length > 0 ? (
            ...
            <p className="text-center text-muted-foreground py-16">
              {isLoadingFeed
                ...
                : searchQuery
                  ? `No visual content found for "${searchQuery}"${selectedChapter !== "all" ? " in this chapter" : ""}`
                  : "No visual content available yet"}
            </p>
          )}
        </TabsContent>
