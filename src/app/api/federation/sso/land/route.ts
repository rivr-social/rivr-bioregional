...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  // AUTH-SEC-002 (F3) — single-use: atomically burn the assertion nonce
  // before minting a session. The assertion travels as a URL query param
  // here, so a captured /sso/land link (history, referer, logs) is especially
  // replayable; the first land wins and any re-presentation falls through
  // unauthenticated.
