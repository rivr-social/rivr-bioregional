...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  // before minting a session. A verified-but-replayed assertion (captured
  // POST body) must not grant a second cookie. Collapses to the same 401 as
  // every verify failure so the route stays a non-oracle.
