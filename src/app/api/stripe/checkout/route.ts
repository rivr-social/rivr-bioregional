...
export async function POST(request: NextRequest) {
  ...
  // membership flows. Unified session so a sovereign's federated remote-viewer
  // members (SSO'd from their home instance, no local NextAuth JWT) can
  // subscribe — plain `auth()` 401'd them (2026-07-11 payment-rail sweep).
  // Federated ids normalize to the local agent and are projected on first
  // contact so createCheckoutSession's keyed writes can't hit "Agent not found".
