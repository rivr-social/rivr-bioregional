...
export async function POST(request: NextRequest) {
  ...
  // Unified session so a federated remote-viewer buyer (SSO'd from their home
  // instance, no local NextAuth JWT) is attributed correctly; anonymous guest
  // checkout still works (null session → null buyer). Federated ids normalize
  // to the local agent.
